...
ct_stream = BitStream()
ct_stream.write(ct, bytes)
ct_stream = ct_stream.read(bool, len(ct) * 8)

gs = [Bool(f"s{i}") for i in range(1000)]
used = 0

def get_var():
    global used
    if used >= len(gs):
        raise RuntimeError("ERROR: used up all generators!")
...
prob = Solver()
state = [get_var() for _ in range(177)]

def step():
    global state
    r = Xor(state[65], state[92])

    v1 = get_var()
    prob.add(And(state[90], state[91]) == v1)
    t1 = Xor(Xor(Xor(state[65], state[92]), state[170]), v1)

    v2 = get_var()
    prob.add(And(state[174], state[175]) == v2)
    t2 = Xor(Xor(Xor(state[68], state[161]), state[176]), v2)
...
def vars(f):
    ...
    return sum([vars(c) for c in f.children()], [])

used_vars = set()
for i in range(300):
    s = step()
    prob.add(s == bool(ct_stream[i] ^ pt[i]))
    for v in vars(s):
        used_vars.add(v)

prob.check()
m = prob.model()
state = [int(bool(m[g])) for g in gs[:177]]

def step_state(out=True):
    global state
    r = state[65] ^ state[92]
...
for i in range(len(ct) * 8):
    r = step_state()
    pt_output.write(r ^ ct_stream[i], bool)

out = pt_output.read(bytes, len(ct))
print(out.decode().strip().split('\n')[-1].strip())
