# relic-maps

**SOLVED**

> Pandora received an email with a link claiming to have information about the location of the relic and attached ancient city maps, but something seems off about it. 
> Could it be rivals trying to send her off on a distraction? 
> Or worse, could they be trying to hack her systems to get what she knows?
> Investigate the given attachment and figure out what's going on and get the flag. 
> The link is to http://relicmaps.htb:/relicmaps.one. 
> The document is still live (relicmaps.htb should resolve to your docker instance).

165.232.100.46:30763
http://relicmaps.htb:/relicmaps.one
http://165.232.100.46:30763/relicmaps.one

```sh
strings relicmaps.one

# Classified Maps
# Created with GIMPW
```

http://165.232.100.46:30763/uploads/soft/topsecret-maps.one
http://165.232.100.46:30763/get/DdAbds/window.bat

`strings topsecret-maps-one`

https://www.dcode.fr/cipher-identifier


```bat
@echo off
set "eFlP=set "
set test=%eFlP%"ualBOGvshk=ws"
echo %test%
:: set "ualBOGvshk=ws"
```

manually decrypted window.bat to decrypt.bat and decrypted.ps1

AES
key=0xdfc6tTBkD+M0zxU7egGVErAsa/NtkVIHXeHDUiW20=
    0xdfc6tTBkD+M0zxU7egGVErAsa/NtkVIHXeHDUiW20=
    d3175f73ab530640fe334cf153b7a019512b02c6bf36d9152075de1c35225b6d

iv=2hn/J717js1MwdbbqMn7Lw==
   2hn/J717js1MwdbbqMn7Lw==
   da19ff27bd7b8ecd4cc1d6dba8c9fb2f

ciphermode=CBC



cyberchef
from base64 
aws decrypt (key in hex, iv in hex, input raw, output hex)
from hex

nope need more decrypting...figured out in `flag.ps1`

`HTB{0neN0Te?_iT'5_4_tr4P!}`